...
public class Shotgun : Weapons {
    ...
    private Ray[] CreatingRays(Enemies.Movement enemy) {
        ...
        Vector3 direction = ToolMethods.SettingVector(enemy.gameObject.transform.TransformDirection(Vector3.forward).x, enemy.directionToTarget.y, enemy.gameObject.transform.TransformDirection(Vector3.forward).z);
        for(int i = 0; i < pellets; i++) {
            rays[i] = new Ray(origin, GetShootingDirection(direction));  
        }
        return rays;
    }
    ...
    private Vector3 GetShootingDirection(Ray mouseRay) {
        Vector3 targetPos = ToolMethods.OffsetPosition(mouseRay.direction * this.shootRange, Random.Range(-spread, spread), Random.Range(-spread, spread), 0);
        Vector3 direction = targetPos - mouseRay.direction;
        return direction;
    }

    private Vector3 GetShootingDirection(Vector3 direction) {
        Vector3 targetPos = ToolMethods.OffsetPosition(direction * this.shootRange, Random.Range(-spread, spread), Random.Range(-spread, spread), 0);
